...
def main(config, image_path, cuda, crf):
    ...
    for i, label in enumerate(labels):
        if classes[label] == 'sky':
            ...
            right_image = cv2.add(origin, np.zeros(np.shape(origin), dtype = np.uint8), mask = mask)
            ax.imshow(right_image)
            cv2.imwrite('demo_sky_masked.jpg', cv2.cvtColor(right_image, cv2.COLOR_BGR2RGB))
